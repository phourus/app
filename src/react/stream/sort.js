...
export default class Sort extends React.Component {
	...
	render() {
		const { params, showSort } = this.props
    const { direction, sortBy } = params

		return (
			<div className={styles.sortby}>
				<div className={styles.toggle} onClick={this._toggle.bind(this)}>Sort by <i className="fa fa-caret-down" /></div>
				<div className={styles.current}>{sortBy} {direction}</div>
				<ul className={showSort ? styles.sort : styles.hide} ref="sort">
				...
				<div className={showSort ? styles.direction : styles.hide}  ref="direction">
	...
	_toggle(e) { this.props.actions.toggleSort() }
